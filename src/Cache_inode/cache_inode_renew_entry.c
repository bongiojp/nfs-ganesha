...
*/
  ...
  LogDebug(COMPONENT_CACHE_INODE,
           "cache_inode_renew_entry use getattr/mtime checking %d, is dir beginning %d, has bit in mask %d, has been readdir %d state %d",
  ...
  if(pentry->internal_md.type == DIR_BEGINNING &&
     ...
     pentry->object.dir_begin.has_been_readdir == CACHE_INODE_YES &&
     ((current_time - entry_time >= pclient->grace_period_dirent) || (pentry->internal_md.valid_state == STALE)))
    {
      ...
      /* Log */
      LogDebug(COMPONENT_CACHE_INODE,
               "Case 1: cached directory entries for entry %p must be renewed (has been readdir)",
               pentry);
...
*/
  ...
  if(pentry->internal_md.type == SYMBOLIC_LINK &&
     ...
     ((current_time - entry_time >= pclient->grace_period_link) || (pentry->internal_md.valid_state == STALE)))
    {
      /* Would be better if state was a flag that we could and/or the bits but in any case we
       * need to get rid of stale so we only go through here once.
       */

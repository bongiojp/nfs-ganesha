...
void setup_dbus(struct _dbus_thread_state *my_thread_state)
{
   ...
   * This way we can setup our own epoll fd that is thread safe. */
  ...
  while (1) {
    ...
    if (watchdata->type == EVENT_WATCH) {
      if (!dbus_watch_get_enabled(watchdata->dbus_watch))
        {
          LogFullDebug(COMPONENT_DBUS, "Received event for disabled "
                       "DbusWatch.");
          continue;
        }

      if (event.events & EPOLLIN) {
        LogFullDebug(COMPONENT_DBUS, "epoll event readable");
        flags |= DBUS_WATCH_READABLE;
      }
      if (event.events & EPOLLOUT) {
        LogFullDebug(COMPONENT_DBUS, "epoll event writeable");
        flags |= DBUS_WATCH_WRITABLE;
      }
      if (event.events & EPOLLHUP) {
        LogFullDebug(COMPONENT_DBUS, "epoll event hangup");
      ...
      if (! dbus_watch_handle(watchdata->dbus_watch, flags)) {
        LogFullDebug(COMPONENT_DBUS, "dbus watch failed to process.");
      } else {
        LogFullDebug(COMPONENT_DBUS, "dbus watch successfully processed");
      }
    } else {
      LogDebug(COMPONENT_DBUS, "A non-Watch event occurred on DBus fd.");
    }

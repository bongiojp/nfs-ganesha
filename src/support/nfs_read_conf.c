  ...
  for(var_index = 0; var_index < var_max; var_index++)
    {
      ...
      if(!strcasecmp(key_name, "Index_Size"))
        ...
        {
          pparam->cid_unconfirmed_hash_param.nb_node_prealloc = atoi(key_value);
          pparam->cid_confirmed_hash_param.nb_node_prealloc = atoi(key_value);
          pparam->cr_hash_param.nb_node_prealloc = atoi(key_value);
        }
      else
        {
          LogCrit(COMPONENT_CONFIG,

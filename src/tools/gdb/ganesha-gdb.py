...
def find_ci_rw_locks():
    ...
    for thread in ganesha_process.threads():
        ...
        while currframe != None:                
            if currframe.name() == "FSAL_read":
                currframe = currframe.older()
                while currframe != None:
                    if currframe.name() == "cache_inode_rdwr":
                        # Make this frame the current frame in GDB
                        currframe.select()
                        lockptr = gdb.parse_and_eval('&entry->content_lock')
                    ...
                    currframe = currframe.older()
            if currframe != None and currframe.name() == "FSAL_write":
                print ""

            if currframe != None and (currframe.name() == "pthread_rwlock_wrlock"
                                      or currframe.name() == "pthread_rwlock_rdlock"):
                if currframe.name() == "pthread_rwlock_wrlock":
                    locktype = "W"
                else: # currframe.name() == "pthread_rwlock_rdlock"
                    locktype = "R"
                ...
                while currframe != None:
                    if currframe.name() == "cache_inode_rdwr":
                        ...
                        if not dictoflocks.has_key(lockptr_str):
                            ...
                            lockdata = gdb.parse_and_eval("entry->content_lock")
...
#        gdb.Command.__init__(self, "print_lock_owners", gdb.COMMAND_DATA, gdb.COMPLETE_SYMBOL, True)
    def invoke(self, arg, from_tty):
        ...
        if len(arg_list) < 1:
            ...
            return
        {'CI_RW': find_ci_rw_locks,
         'BACK': printbacktrace,
         }.get(arg_list[0], functionnotfound)()

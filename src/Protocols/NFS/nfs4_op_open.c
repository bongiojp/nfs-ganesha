...
static void finish_deleg_reply(OPEN4resok *resok, open_delegation_type4 deleg_type,
			   ...
			   nfs_client_id_t *client, OPEN4resok *resok)
{
	...
	if (state_status != STATE_SUCCESS) {
		...
		if (state_status != STATE_SUCCESS) {
			...
			finish_deleg_reply(resok, deleg_type, new_state,
					   data->current_entry);
		}
	}
			  ...
			  state_t *open_state, nfs_client_id_t *clientid)
{
	...
	glist_for_each(glist, &data->current_entry->state_list) {
		...
		if (state_iterate->state_type == STATE_TYPE_DELEG &&
		    state_iterate->state_owner->so_owner.so_nfs4_owner.so_clientid
		    == clientid->cid_clientid) {
			LogFullDebug(COMPONENT_STATE, "Reusing existing DELEG state.");
			/* We'll be re-using the found state */
			finish_deleg_reply(resok,
					   state_iterate->state_data.deleg.sd_type,

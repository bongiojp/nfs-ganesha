			   ...
			   nfs_client_id_t *client, OPEN4resok *resok)
{
	...
	struct state_refer refer;
	state_t *new_state, *found_state;
	struct glist_head *glist;
	state_lock_entry_t *found_lock = NULL;
	...
	glist_for_each(glist, &data->current_entry->object.file.deleg_list) {
		found_lock = glist_entry(glist, state_lock_entry_t, sle_list);
		found_state = found_lock->sle_state;
		if (found_state->state_owner->so_owner.so_nfs4_owner.so_clientid
		    == client->cid_clientid) {
			LogFullDebug(COMPONENT_STATE,
				     "Reusing existing delegation.");
			assert(found_state->state_data.deleg.sd_type
			       == OPEN_DELEGATE_WRITE ||
			       (args->share_access & OPEN4_SHARE_ACCESS_WRITE)
			       != 1);
			/* We'll be re-using the found state */
			fill_deleg_reply(resok,
					 found_state->state_data.deleg.sd_type,
					 found_state, data->current_entry);
			return;
		}
	}

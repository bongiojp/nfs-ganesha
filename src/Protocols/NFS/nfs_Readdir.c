            ...
            nfs_res_t *res)
{
     ...
     int rc = NFS_REQ_OK;
     struct nfs2_readdir_cb_data cb2;
     struct nfs3_readdir_cb_data cb3;
     ...
     if ((req->rq_vers == NFS_V3) &&
         (nfs3_Is_Fh_Xattr(&(arg->arg_readdir3.dir)))) {
          rc = nfs3_Readdir_Xattr(arg, export, context, client, req, res);
          goto out;
     }
...
<<<<<<< HEAD
     ...
     /* Allocate memory and fill out the callback opaques */
     if (req->rq_vers == NFS_V2) {
          ...
          cb3.mem_left = count - sizeof(READDIR2resok);
          cb3.count = 0;
          cb3.context = context;
          cb3.error = NFS_OK;
     }
...
>>>>>>> aa89dfc... Handle NFSv[23] input cookie=2 case correctly.
                      ...
                      uint64_t cookie)
{
     ...
     strcpy(tracker->entries[tracker->count].name,
            name);
     memcpy(tracker->entries[tracker->count].cookie,
            &truncookie,
            NFS2_COOKIESIZE);
     if (tracker->count != 0) {

...
bool update_delegation_stats(struct deleg_data *deleg_entry)
{
	...
	atomic_inc_uint32_t(&client->curr_deleg_grants);
...
bool deleg_heuristics_recall(struct deleg_data *deleg_entry)
{
	...
	atomic_dec_uint32_t(&client->curr_deleg_grants);
			   ...
			   state_owner_t *owner, bool *prerecall)
{
	...
	*prerecall = false;
	...
	if (atomic_fetch_uint32_t(&client->cid_deleg_stats.num_revokes) > 2) /* more than 2 revokes */
		return false;
